# 第五章 架构 (Space-Based Architecture)
...
如图 5-3 所示，消息传递网格用于管理输入请求和会话信息。当请求到达虚拟化中间件组件时，消息传递网格组件会确定哪些活动处理组件可以接收该请求，并将请求转发给其中一个处理单元。消息传递网格的复杂程度可以从简单的轮询算法到更复杂的“下一个可用”算法不等，后者会跟踪哪个请求正在由哪个处理单元处理。
